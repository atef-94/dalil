import { el, clear, table, toast, errorBanner, selectInput, confirmAction } from '../ui.js';
        ...
        [
          ...
          { label: '', render: (g) => {
            ...
            btn.addEventListener('click', async () => {
              if (!confirmAction('Revoke this grant?')) return;
              try {
  ...
  async function loadAssignCard() {
    ...
    try {
      ...
      async function loadAssignments() {
        ...
        const userRoles = await api.get(`/api/users/${userSelect.value}/roles`);
